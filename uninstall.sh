...
# Common function to backup database
Backup_Database()
{
    if [ ${DB_Name} != "None" ]; then
        Remove_StartUp ${DB_Name}
        backup_dir="/root/databases_backup_$(date +"%Y%m%d%H%M%S")"
        echo "Backup ${DB_Name} databases directory to ${backup_dir}"
...
# Common function to remove database files
...
# Common function to uninstall acme.sh
Uninstall_Acme_Sh()
{
