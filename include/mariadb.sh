...
MariaDB_WITHSSL()
{
    # NOTE: SSL configuration for MariaDB is currently not implemented
    # TODO: Add SSL/TLS configuration options for MariaDB connections
    MariaDBWITHSSL=''
}
...
password=''
EOF
        if [[ "${DBSelect}" =~ ^[23]$ ]] || echo "${mariadb_version}" | grep -Eqi '^10.[4-7].'; then
...
Install_MariaDB_105()
{
    if [ "${Bin}" = "y" ]; then
        ...
        # Check disk space before compilation (requires ~5GB)
        local available_space=$(df -BG ${cur_dir}/src | tail -1 | awk '{print $4}' | tr -d 'G')
        if [ "${available_space}" -lt 5 ]; then
            Echo_Red "Error: Insufficient disk space for MariaDB compilation!"
            Echo_Red "Available: ${available_space}GB, Required: at least 5GB"
            Echo_Red "Please free up disk space or use binary installation instead."
            exit 1
        fi
        rm -f /etc/my.cnf
