...
Check_Hosts()
{
    ...
    if [ "${CheckMirror}" != "n" ]; then
        pingresult=`ping -c1 github.com 2>&1`
        echo "${pingresult}"
...
Check_Download()
{
    ...
    if [[ "${DBSelect}" =~ ^[123]$ ]]; then
        ...
        if [ "${Bin}" = "y" ]; then
            ...
            Download_Files https://archive.mariadb.org/mariadb-${Mariadb_Version}/bintar-linux-systemd-${DB_ARCH}/mariadb-${Mariadb_Version}-linux-systemd-${DB_ARCH}.tar.gz ${Mariadb_Ver}-linux-systemd-${DB_ARCH}.tar.gz
        else
            Download_Files https://archive.mariadb.org/mariadb-${Mariadb_Version}/source/mariadb-${Mariadb_Version}.tar.gz ${Mariadb_Ver}.tar.gz
        fi
    fi
    # Debug: Check Php_Ver before download
    if [ -z "${Php_Ver}" ]; then
        Echo_Red "Error: Php_Ver is empty! Please check version.sh loading."
        Echo_Red "PHPSelect = ${PHPSelect}"
        exit 1
    fi
    Echo_Blue "[+] PHP Version: ${Php_Ver}"
    ...
    Download_Files ${Download_Mirror}/prober/probe.zip probe.zip
    if [ "${Stack}" != "lnmp" ]; then
